...
class HTTFitConstantBank : public AthMessaging
{
  ...
  HTTFitConstantBank(MsgStream & msg, HTTPlaneMap const * pmap, int ncoords, std::string const & fname, bool isFirstStage, int missingPlane = -1);
	...
	vector2D<int> m_WCs; //[m_nsectors][m_ncoords] which coordinates are WCs
        ...
        float **m_maj_a; // offset for missing coordinate
        float ***m_maj_kk; // for majority fits
        float ***m_maj_invkk; // for majority inverse
        */
        ...
        void readHeader(htt_dcap::istream & geocfile);
        void readSectorInfo(htt_dcap::istream & geocfile);
