...
HTTFitConstantBank::HTTFitConstantBank(MsgStream & msg, HTTPlaneMap const * pmap, int ncoords, std::string const & fname, bool isFirstStage, int missingPlane) :
    AthMessaging(msg),
    m_pmap(pmap),
    m_bankID(0),
    m_ncoords(ncoords),
    ...
    m_isIdealCoordFit(false)
{
    ...
    htt_dcap::istream geocfile;
    const bool ok_read = htt_dcap::open_for_read(fname.c_str(), geocfile);
    if (!geocfile || !ok_read) ATH_MSG_FATAL("FitConstants file: " << fname << " invalid");
...
void HTTFitConstantBank::readHeader(htt_dcap::istream & geocfile)
{
    string key;
    int ival;
    int nplanes;
    ...
    geocfile >> key;
    if (key != "NPLANES") ATH_MSG_FATAL("Invalid file format");
    geocfile >> nplanes;

    geocfile >> key;
    if (key!="NSECTORS") ATH_MSG_FATAL("Invalid file format");
    geocfile >> m_nsectors;

    geocfile >> key;
    if (key!="NDIM") ATH_MSG_FATAL("Invalid file format");
    geocfile >> ival;

    // Set derived configuration variables
    if (ival==1) m_npars = 3;
    else if (ival==2) m_npars = 5;
    else ATH_MSG_FATAL("Number of dimensions invalid");
...
void HTTFitConstantBank::readSectorInfo(htt_dcap::istream & geocfile)
{
    string key;
    int ival;
    ...
    for (int isec = 0; isec < m_nsectors; ++isec)
    {
        // Read sector number
        geocfile >> key >> ival;
        if (ival != isec) ATH_MSG_ERROR("Reached sector " << ival << " but expected sector " << isec);
	  ...
	  //for (int ipar = 0; ipar < 5; ++ipar)
        {
            ...
            for (int icoord = 0; icoord < m_ncoords; ++icoord)
            {
                geocfile >> dval;
                if (geocfile.fail())
                    ATH_MSG_ERROR("par loop (1) key=\"" << key << "\" ipar,icoord=" << ipar << "," << icoord);
...
bool HTTFitConstantBank::linfit(sector_t sector, HTTTrack & track, bool isSecondStage) const
{
    ...
    // Debug
    if (false && msgLvl(MSG::DEBUG)) // This is segfaulting
    {
        stringstream s;
        s << "Coords: ";
        for (unsigned ip=0;ip<track.getHTTHits().size();++ip) {
           s << setw(13) << track.getPhiCoord(ip); // segfault here
           if (m_pmap->getDim(ip) == 2) s << setw(13) << track.getEtaCoord(ip);
        }
        s << "\n";
        s << "\t\t\tReconstructed parameters\n\t\t\t" \
            << track.getQOverPt() << ' ' << track.getD0() \
            << ' ' << track.getPhi() << ' ' << track.getZ0() \
            << ' ' << track.getEta() << "\n";
        s << "\t\t\tChi2 " << track.getChi2() << "\n";
        ATH_MSG_DEBUG(s.str());
    }
    return true;
}
...
{
    vector<float> pars(m_npars);

    for (int ip = 0; ip < m_npars; ip++)
    {
        // pars[ip] = TMath::Power(2,-13)*m_fit_const[sector, ip];
