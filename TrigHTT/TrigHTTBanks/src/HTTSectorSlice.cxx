...
void largest_region_wrap(vector<bool> const & good_bin, int & i_lower, int & i_upper);
...
HTTSectorSlice::HTTSectorSlice(MsgStream & msg, size_t nSectors,
...
HTTSectorSlice::HTTSectorSlice(MsgStream & msg, std::string const & filepath) :
    AthMessaging(msg)
{
...
vector<sector_t> HTTSectorSlice::searchSectors(HTTTrackPars const & pars) const
{
    vector<sector_t> sectors;
    if (!checkTrackPars(pars))
    {
        ATH_MSG_ERROR("searchSectors() Bad track parameters: " << pars);
        return sectors;
    }
...
std::pair<HTTTrackPars, HTTTrackPars> HTTSectorSlice::getBoundaries() const
{
    ...
    ATH_MSG_DEBUG("getBoundaries() returning " << min << "; " << max);
    return { min, max };
}
        ...
        double &bound_min, double &bound_max, bool wraps, const char *debug) const
{
    ...
    if (i_lower >= i_upper) ATH_MSG_ERROR("getBoundary() Completely empty bitmask array?");
    bound_min = x_min + ((x_max-x_min) * i_lower) / nbin;
    bound_max = x_min + ((x_max-x_min) * (i_upper+1)) / nbin;

    ATH_MSG_DEBUG("getBoundary() " << debug << " nbin=" << nbin
            << " range=[" << x_min << ", " << x_max
...
void largest_region_wrap(vector<bool> const & good_bins, int & i_lower, int & i_upper)
{
