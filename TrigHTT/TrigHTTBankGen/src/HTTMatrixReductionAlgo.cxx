...
{
    declareProperty("LayerChange_path", m_filePath, "extract1stStage");
...
StatusCode HTTMatrixReductionAlgo::initialize()
{
    ...
    TFile *file = TFile::Open(m_filePath.c_str(), "");
...
{
    // Do nothing. This class does not process events. Main functionality is
    // called from initialize().
    return StatusCode::SUCCESS;
}


// What is inversion here? Seems to be:
// Switches end cap strip layer sides when matching 1st stage to 2nd stage layers.
// The inversion factor is added to the layer and coordinate index of the second stage.
int HTTMatrixReductionAlgo::getInversion(size_t layer_2nd)
{
    // This doesn't make sense at all. How do you know there's a second section? What is the point of this?
    // In any case, the order of these checks is all wrong. Should check if strip endcap first, then check section.
    /*
    bool doInvert = m_pmap_2nd->getModuleInfo(layer_2nd, 0).stereo != m_pmap_2nd->getModuleInfo(layer_2nd, 1).stereo;
    if (m_pmap_2nd->getDim(layer_2nd) != 1) doInvert = false; // Only invert strips
    if (doInvert && m_pmap_2nd->isEC(layer_2nd, 0)) // TODO section number?
    {
        if (layer_2nd % 2 == 0) return 1;
        else return -1;
    }
    */
    (void) layer_2nd;
...
std::pair<std::vector<size_t>, std::vector<size_t>> HTTMatrixReductionAlgo::matchStages()
{
    ...
    for (unsigned layer_1st = 0; layer_1st < m_pmap_1st->getNLogiLayers(); layer_1st++)
    {
        ...
        int inversion = getInversion(ls_2nd.layer);
