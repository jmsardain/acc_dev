...
class HTTHit :
    """this class describe the content of a single hit reading the format of the HTT wrapper"""
    def __init__(self,raw_elements) :
        ...
        else : # endcap part, look HTTPMap.cxx for further details
            outside = 0
            inner_ring = 0;
...
class HTTTruth :
    def __init__(self,params) :
        """HTTTruth is described in the files produced by the wrapper using 8 values, here the value are converted in the ones used in HTT"""
...
class HTTEvent :
    def __init__(self, outFile=None) :
        ...
        if outFile:
            self.EventStreamer = HTTEventStreamer(outFile)
        else:
        ...
        for line in stream :
            ...
            if line[0] == "R" :
                ...
                if not self.ignoreHits :
                    self.raw_hits.append(HTTHit(line.split()[1:]))
            elif line[0] == "E" :
                if not self.ignoreOffline :
                    self.off_tracks.append(HTTOffline(line.split()[1:]))
            elif line[0] == "T" :
                if not self.ignoreTruth :
                    self.truth_tracks.append(HTTTruth(line.split()[1:]))
            elif line[0] == "L" :
                break
        return inEvent
...
class HTTFileChain :
    def __init__(self,filelist,outFile=None) :
        ...
        self.outFile = outFile
        self.HTTEvent = HTTEvent(self.outFile)
    ...
    def nextevent(self) :
        ...
        self.HTTEvent.Reset()
        while not self.HTTEvent.ReadFromFile(self.stream) :
            self.currentID += 1
