#!/bin/bash
# This test simply runs HTTOutputMonitorAlg_tf to make sure it runs without error
...
withPU=false
HTTLogHitFile="loghit_direct.root"
...
HTTLogicalHitsToAlg_tf.py \
    ...
    --OutFileName $HTTLogHitFile \
    --sampleType $sampleType \
    --withPU $withPU \
    --bankTag 'EF_TaskForce_dev22' \
    --region $region \
    --secondStage $secondStage \
    > transform_output1.txt

retVal=$?

echo "Running on ${HTTLogHitFile}"
echo "Processing finished with status = $retVal; now read back"
...
HTTOutputMonitorAlg_tf.py \
    ...
    --region $region \
    --secondStage $secondStage \
    > transform_output2.txt

retVal=$?

sed 's/^.\{,9\}//' log.HTTOutputMonitorAlg
echo "OutputMonitor finished with status = $retVal"
