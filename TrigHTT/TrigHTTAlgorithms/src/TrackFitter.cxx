...
TrackFitter::TrackFitter(MsgStream & msg, HTTFitConstantBank *nominalbank,
        std::vector<HTTFitConstantBank*> droppedLayerbanks, bool guessingHits) :
    AthMessaging(msg),
    ...
    m_nominalBank(nominalbank)
{
    ...
    if (!m_guessinghits)
        for (auto bank : droppedLayerbanks) m_droppedLayerBanks.push_back(bank);
}
...
void TrackFitter::init()
{
    if (m_identify_badhit)
    {
        ...
        m_nominalBank->prepareInvFitConstants(); // <-- what exactly does this do?
    }
}
...
{
    tracks_missinghits_track.clear();
        ...
        if (getDoMissingHitsCheck()) {
            if (track_cands[icomb].getNMissing() == 0) { // missing 0 hits, drop hits one at a time and refit!
                for (unsigned icoord = 0; icoord < m_pmap->getNCoords(); icoord++) { // 9 coordinates to refit, nominally
                    ...
                    tracks_missinghits_track.push_back(newtrack);
 ...
 * over `combs`, return a single track each call. However, this is strongly dependent on the number
 * of hits that can fit in a single ssid, which shouldn't be that large.
 */
...
HTTTrack TrackFitter::recoverTrack(HTTTrack const & t, sector_t sector, layer_bitmask_t norecovery_mask, double qoverpt)
{
    ...
    vector<HTTTrack> recovered_tracks(m_pmap->getNLogiLayers(),t); // start with the complete track in all layers
    ...
    for (unsigned layer = (m_require_first ? 1 : 0); layer < recovered_tracks.size(); ++layer)
    {
        ...
        int ix = m_pmap->getCoordOffset(layer);
        // unsigned int missing_mask = t.getHitMap() | (1 << ix);
...
// Returns the index in tracks with the best chi2
std::vector<HTTTrack>::const_iterator getBestChi2ndof(std::vector<HTTTrack> const & tracks)
{
    float best_chi2ndof = std::numeric_limits<float>::infinity();
    auto best_i = tracks.end();

    for (auto it = tracks.begin(); it != tracks.end(); it++)
    {
        float chi2ndof = it->getChi2ndof();
        if (chi2ndof > 0 && chi2ndof < best_chi2ndof)
        {
            best_chi2ndof = chi2ndof;
            best_i = it;
        }
    }
...
{
    for (unsigned i = 0; i < track_cands.size(); i++)
    {
        HTTTrack const & t = track_cands[i];
