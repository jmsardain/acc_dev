...
class HTTEtaPatternFilterTool::ModulesToPattern
{
    public:
    ...
    m_HTTMapping("TrigHTTMappingSvc", name)
{
    ...
    declareProperty("EtaPatterns", m_pattern_file_path, "where to get the eta patterns");
...
StatusCode HTTEtaPatternFilterTool::finalize()
{
    return StatusCode::SUCCESS;
}
...
std::string HTTEtaPatternFilterTool::to_string(std::vector<unsigned> v) const
{
...
std::string HTTEtaPatternFilterTool::to_string(EtaPattern patt) const
{
    std::ostringstream oss;
    oss << "[";
    for (auto mod : patt) {
...
std::string HTTEtaPatternFilterTool::to_string(HTTRoad road) const
{
    std::ostringstream oss;
    oss << road.getNHits()  << " : [";
    for (unsigned layer = 0; layer < m_nLayers; layer++) {
