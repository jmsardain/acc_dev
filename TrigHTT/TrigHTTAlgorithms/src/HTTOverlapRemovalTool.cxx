...
static const InterfaceID IID_HTTOverlapRemovalTool("HTTOverlapRemovalTool", 1, 0);
const InterfaceID& HTTOverlapRemovalTool::interfaceID()
{ return IID_HTTOverlapRemovalTool; }
...
{
    declareInterface<HTTOverlapRemovalTool>(this);
    declareProperty("DoSecondStage", m_do2ndStage=false, "Do second stage of not?");
    declareProperty("NumOfHitPerGrouping", m_NumOfHitPerGrouping=5, "Number of hits pre grouping"); //Default is set for normal grouping
    declareProperty("MinChi2", m_minChi2=40, "Minimum chi2 requirement for tracks being considered in OR");
    declareProperty("roadSliceOR", m_roadSliceOR, "Do slice-wise overlap removal for roads (Hough-only)");
    ...
    declareProperty("ORAlgo", m_algorithm="Normal", "Overlap removal algorithm");
    declareProperty("doFastOR", m_doFastOR, "Use fast overlap removal algorithm instead of default");
}
...
StatusCode HTTOverlapRemovalTool::initialize()
{
  ...
  //  Setup OR algorithm
  if(m_algorithm.value() == "Normal") m_algo=ORAlgo::Normal;
  else if(m_algorithm.value() == "Invert") m_algo=ORAlgo::InvertGrouping;
  else
  {
...
StatusCode HTTOverlapRemovalTool::runOverlapRemoval(std::vector<HTTRoad*>& roads)
{
    ...
    for (HTTRoad* r: roads)
    {
        ...
        if (!hough) return StatusCode::FAILURE; // mixed hough non-hough roads?
...
int HTTOverlapRemovalTool::findNCommonHits(const HTTTrack& Track1, const HTTTrack& Track2)
{
  ...
  for(unsigned int i = 0; i < Track1.getHTTHits().size(); ++i)
  {
    // Check if hit is missing
    if(!Track1.getHTTHits().at(i).isReal() || !Track2.getHTTHits().at(i).isReal())
    {
      continue;
    }
    // Check if hit on the same plane
    else if(Track1.getHTTHits().at(i).getLayer() != Track2.getHTTHits().at(i).getLayer())
    {
      continue;
    }
    // Check if two hits have the same hashID
    else if(Track1.getHTTHits().at(i).getHTTIdentifierHash() != Track2.getHTTHits().at(i).getHTTIdentifierHash())
    {
      continue;
    }
    // Check if two hits have same coordinate
    else if(Track1.getHTTHits().at(i).getPhiCoord() == Track2.getHTTHits().at(i).getPhiCoord()
            && Track1.getHTTHits().at(i).getEtaCoord() == Track2.getHTTHits().at(i).getEtaCoord())
    {
      nCommHits++;
    }
    else
...
StatusCode HTTOverlapRemovalTool::finalize()
{
  ATH_MSG_INFO( "HTTOverlapRemovalTool::finalize()" );
