...
static const InterfaceID IID_HTTDataFlowTool("HTTDataFlowTool", 1, 0);
const InterfaceID& HTTDataFlowTool::interfaceID()
{ return IID_HTTDataFlowTool; }
...
{
    declareInterface<HTTDataFlowTool>(this);
                                              ...
                                              std::vector<HTTRoad*> const & roads_2nd, std::vector<HTTTrack> const & tracks_2nd)
{
    ...
    std::unordered_map<int, size_t> m_pattID_nTracks_1st;
    std::unordered_map<int, size_t> m_pattID_nTracks_1st_passChi2;
    std::unordered_map<int, size_t> m_pattID_nTracks_1st_afterOR;

    std::unordered_map<int, size_t> m_sectID_nTracks_1st;
    std::unordered_map<int, size_t> m_sectID_nTracks_1st_passChi2;
    std::unordered_map<int, size_t> m_sectID_nTracks_1st_afterOR;
    ...
    for (HTTTrack const & t : tracks_1st) {
        if (t.getChi2ndof() <= m_cut_chi2ndof) {
            ...
            m_pattID_nTracks_1st_passChi2[t.getPatternID()]++;
            m_sectID_nTracks_1st_passChi2[t.getFirstSectorID()]++;
        }
        if (t.passedOR() == 1) {
            info->nTracks_1st_afterOR++;
            m_pattID_nTracks_1st_afterOR[t.getPatternID()]++;
            m_sectID_nTracks_1st_afterOR[t.getFirstSectorID()]++;
        }
        m_pattID_nTracks_1st[t.getPatternID()]++;
        m_sectID_nTracks_1st[t.getFirstSectorID()]++;
    }
    for (std::pair<int, size_t> p : m_pattID_nTracks_1st) {
        info->nTracks_1st_per_patt_total.push_back(p.second);
    }
    for (std::pair<int, size_t> p : m_pattID_nTracks_1st_passChi2) {
        info->nTracks_1st_per_patt_passChi2.push_back(p.second);
    }
    for (std::pair<int, size_t> p : m_pattID_nTracks_1st_afterOR) {
        info->nTracks_1st_per_patt_afterOR.push_back(p.second);
    }
    info->nConstants_1st_fitter = m_sectID_nTracks_1st.size();
    info->nConstants_1st_passChi2 = m_sectID_nTracks_1st_passChi2.size();
    info->nConstants_1st_afterOR = m_sectID_nTracks_1st_afterOR.size();

    if (m_runSecondStage) {
        info->nRoads_2nd_total = roads_2nd.size();

        std::unordered_map<int, size_t> m_sectID_nTracks_2nd;
        std::unordered_map<int, size_t> m_sectID_nTracks_2nd_passChi2;
        std::unordered_map<int, size_t> m_sectID_nTracks_2nd_afterOR;

        info->nTracks_2nd_total = tracks_2nd.size();
        for (HTTTrack const & t : tracks_2nd) {
            if (t.getChi2ndof() <= m_cut_chi2ndof) {
                info->nTracks_2nd_passChi2++;
                m_sectID_nTracks_2nd_passChi2[t.getSecondSectorID()]++;
            }
            if (t.passedOR() == 1) {
                info->nTracks_2nd_afterOR++;
                m_sectID_nTracks_2nd_afterOR[t.getSecondSectorID()]++;
            }
            m_sectID_nTracks_2nd[t.getSecondSectorID()]++;
        }

        info->nConstants_2nd_fitter = m_sectID_nTracks_2nd.size();
        info->nConstants_2nd_passChi2 = m_sectID_nTracks_2nd_passChi2.size();
        info->nConstants_2nd_afterOR = m_sectID_nTracks_2nd_afterOR.size();
    }
...
StatusCode HTTDataFlowTool::printDataFlow(std::string const & key, int const div)
{
    ...
    if (key.find("_2nd") != std::string::npos) {
        str_stage = "SECOND";
