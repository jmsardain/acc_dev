    ...
    m_monitorFile((m_outFileName.value() + ".root").c_str(), "RECREATE")
{
    ...
    declareProperty("OutFileName",                  m_outFileName,           "naming convention for maps");
    declareProperty("region",                       m_region);
    declareProperty("KeyString",                    m_keystring,             "key layer to use for subrmap");
    declareProperty("KeyString2",                   m_keystring2,            "second key layer for 2D slicing");
    declareProperty("nSlices",                      m_nSlices,             "default is full granularity/maximum number of slices possible");
    declareProperty("maxEvents",                    m_maxEvents);
    declareProperty("trim",                         m_trim,                   "trim modules with less than given percent of tracks");
}
...
StatusCode HTTMapMakerAlg::writeSubrmap(std::vector<HTTHit> const & allHits)
{
   ...
   // easier to use vector than set, convert m_key_etamods into key_etas
    ...
    ATH_MSG_INFO("Nslices = " << std::to_string(m_nSlices) << ":");
    ...
    { // make new slices from combinations of keylayer1 slices and keylayer2 slices
        ...
        ATH_MSG_INFO("These slices were further divided based on the key layer 2: '" << key2 << "'. Now nSlices = " << m_nSlices);
