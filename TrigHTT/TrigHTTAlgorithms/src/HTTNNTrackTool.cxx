...
{
    //    declareProperty("NNCut", m_NNCut = 0.02);
    declareProperty("NNCut", m_NNCut = 0.0);
   
    // Nominal chi2ndof cut is 40 and we want to use NN>0.0075 (or NN<(1-0.0075)
    declareProperty("Chi2ScaleFactor", m_chi2_scalefactor = 40/(1-0.1));
}
...
{
  // ECC - need to add in bank service here!
...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
StatusCode HTTNNTrackTool::finalize()
{
    return StatusCode::SUCCESS;
}
                                     ...
                                     const HTTNNMap *nnMap)
{
  ...
  for (auto iroad : roads){
    ...
    // Code from TrackFitter.cxx //////////////////////////////////////////////
    // ECC tracks_missinghits_track.clear();
    ...
    // ECC    getMissingInfo(*iroad, nMissing, missPixel, missStrip, missing_mask, norecovery_mask);
    ...
    temp.setQOverPt(y);
    // ECC  temp.setTrackCorrType(m_IdealCoordFitType);
    ...
    for (size_t icomb = 0; icomb < combs.size(); icomb++)  {
      ...
      std::sort(hit_list.begin(), hit_list.end(), [](const HTTHit* hit1, const HTTHit* hit2) {
        double rho1 = hit1->getX()*hit1->getX() + hit1->getY()*hit1->getY();
        double rho2 = hit2->getX()*hit2->getX() + hit2->getY()*hit2->getY();
        return rho1 < rho2;
      });
      ...
      for(const auto& hit: hit_list) {
        ...
        if (index == 1) {
          if (z0 < 0) flipZ = true;
          rotateAngle = atan(x0/y0);
          if(y0 < 0) rotateAngle += 3.14159;
        }	  

        double xf = x0 * cos(rotateAngle) - y0 * sin(rotateAngle);
        double yf = x0 * sin(rotateAngle) + y0 * cos(rotateAngle);
        double zf = z0; if (flipZ) zf = z0*-1;
