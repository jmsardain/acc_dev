...
class TrackFitter : public AthMessaging
{
    public:
       ...
       int fitTracks(const std::vector<HTTRoad*>& roads, std::vector<HTTTrack>& tracks);
        ...
        std::vector<HTTTrack> getMissingHitsCheckTracks() {return tracks_missinghits_track;}

        ///////////////////////////////////////////////////////////////////////
        // Constructor and Initialization

        TrackFitter(MsgStream & msg, HTTFitConstantBank *nominalbank, std::vector<HTTFitConstantBank*> droppedLayerbanks, bool guessingHits);
        void init();
	...
	void setIdealCoordFitType(TrackCorrType v) { 
	  m_IdealCoordFitType = v; 
	  if (m_IdealCoordFitType != TrackCorrType::None) {
	    m_nominalBank->setIdealCoordFit(true);
	    for (auto droppedbank : m_droppedLayerBanks) droppedbank->setIdealCoordFit(true);
	  }
	  else {
	    m_nominalBank->setIdealCoordFit(false);
	    for (auto droppedbank : m_droppedLayerBanks) droppedbank->setIdealCoordFit(false);
	  }
	}
	...
	bool m_doDeltaGPhis = false;
        ...
        // It is just a pointer from BankSvc->FitConstantBank1stStage()
        HTTFitConstantBank *m_nominalBank;

        // These are only used if we are not guessing hits, one for each plane missing
        // Also just a set of pointers
        std::vector<HTTFitConstantBank*> m_droppedLayerBanks;

        // These are the tracks we store if we are doing the missing hits cross-check
        std::vector<HTTTrack> tracks_missinghits_track;
