...
print("reading track input: %d entries\n" % (num_entries,))
t0 = time.clock()
...
def getRow(trk,hit_idx): # returns localrow,globalrow
    ...
    if trk.SensitiveIsPixel[hit_idx] == 0:
        ...
        if localrow > rows_in_module:
           print("Error row(",localrow,") greater than number of rows in module,",rows_in_module)
...
def parseGeometry():
    ...
    print(len(eta_vals_for_key_layer)," eta rows:",sorted(eta_vals_for_key_layer))
    print("Tracks Used=",tracks_used)
...
def divideKeyLayerModulesIntoSlices(eta_vals_for_key_layer):
    ...
    print("Modules in slices",key_modules_for_slices)
...
def getModuleToSliceMap(mod_slice_map):
    ...
    print("Tracks Used=",tracks_used)
    print(str(missing_keylayer_hit)+" tracks with no keylayer hit found or multiple hits in keylayer")
...
def printOccupancies(slice_modules):
    ...
    print("%-5s %-6s %-5s %-12s %-10s %-10s"  % ("","","","","min/max","min/max",))
    print("%-5s %-6s %-5s %-12s %-10s %-10s"  % ("det","reg","lyr","duplication","modules","occupancy",))
    for det in dets:
        for reg in regs:
            for lyr in range(layers[det][reg]):
                dup=-1
                ...
                print("%5s %5s %-5d    %2.2f      [%2d,%2d]     [%2.2f,%2.2f]"  % (det,reg,lyr,dup,min(lens),max(lens),min(occs),max(occs)))
                #print(("%5s %5s %3d"+" %2.3f"*nslices) % tuple([det,reg,lyr]+occs))
                #print(("%5s %5s %3d"+" %3d"*nslices) % tuple([det,reg,lyr]+lens))
...
if key2:
    print(len(slice_modules)," two layers slices found")
