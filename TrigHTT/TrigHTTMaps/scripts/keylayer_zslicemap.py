...
# create rmap file
#rmap = open('01eta03_03phi05_kl_strip2_ATLAS-P2-ITK-22-02-00.rmap', 'w')
...
for i in range(0,6):
    ...
    try:
        ...
        print("pixel barrel had no hits in slice %d" % i)
        pbmax = -1
    try:
        pemax = max(int(l['layer']) for l in pe.values()) + 1
    except ValueError:
        print("pixel endcap had no hits in slice %d" % i)
        pemax = -1
    try:
        sbmax = max(int(l['layer']) for l in sb.values()) + 1
    except ValueError:
        print("strip barrel had no hits in slice %d" % i)
        sbmax = -1
    try:
        semax = max(int(l['layer']) for l in se.values()) + 1
    except ValueError:
        print("strip endcap had no hits in slice %d" % i)
        semax = -1
