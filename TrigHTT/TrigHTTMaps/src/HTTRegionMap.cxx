...
HTTRegionMap::HTTRegionMap(MsgStream & msg, const HTTPlaneMap *pmap, std::string const & filepath) :
    AthMessaging(msg),
    m_pmap(pmap)
{
    // Open the file
...
void HTTRegionMap::allocateMap(ifstream & fin)
{
    ...
    if (!ok) ATH_MSG_FATAL("Error reading header");
...
{
    ATH_MSG_INFO("Reading module LUT" << filepath);
    ifstream fin(filepath);
    if (!fin.is_open())
    {
        ATH_MSG_ERROR("Couldn't open " << filepath);
        throw ("HTTRegionMap Couldn't open " + filepath);
    }
    ...
    while (getline(fin, line))
    {
        ...
        if (!(sline >> region >> layer >> globalID >> localID))
            ATH_MSG_WARNING("Error reading module LUT");
        else if (region >= m_global_local_map.size() || layer >= m_pmap->getNLogiLayers())
            ATH_MSG_WARNING("loadModuleIDLUT() bad region=" << region << " or layer=" << layer);
        else
...
uint32_t HTTRegionMap::getUnmappedID(uint32_t region, const HTTHit &hit) const
{
  ...
  int phi          = hit.getPhiModule();
    ...
    //if (hit.getASide() || hit.getCSide()) err[0] = 2;
...
uint32_t HTTRegionMap::getLocalID(uint32_t region, uint32_t layer, uint32_t globalModuleID) const
{
    ...
    {
        ATH_MSG_ERROR("getLocalID() bad region=" << region << " or layer=" << layer);
        return -1;
    }

    auto it = m_global_local_map[region][layer].find(globalModuleID);
    if (it == m_global_local_map[region][layer].end())
    {
        ATH_MSG_ERROR("getLocalID() Did not find local id for region " << region << ", layer " << layer << ", globalID " << globalModuleID);
        return -1;
    }
...
uint32_t HTTRegionMap::getGlobalID(uint32_t region, uint32_t layer, uint32_t localModuleID) const
{
    if (region >= m_global_local_map.size() || layer >= m_pmap->getNLogiLayers())
    {
        ATH_MSG_ERROR("getGlobalID() bad region=" << region << " or layer=" << layer);
        return -1;
    }
    ...
    ATH_MSG_ERROR("getGlobalID() Did not find global id for region " << region << ", layer " << layer << ", localID " << localModuleID);
    return -1;
}
