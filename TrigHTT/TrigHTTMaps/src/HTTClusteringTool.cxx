...
void splitAndSortHits(std::vector<HTTHit>& hits, std::vector<std::vector<HTTHit> >& hitsPerModule, int& eta_phi);
void splitAndSortHits(std::vector<HTTHit>& hits, std::vector<std::vector<HTTHit> >& hitsPerModule);
void splitHitsToModules(std::vector<HTTHit>& hits, std::vector<std::vector<HTTHit> >& hitsPerModule);
...
void normaliseClusters(std::vector<HTTCluster> &clusters);
void sortHitsOnModules(std::vector<std::vector<HTTHit> >& hitsPerModule, int& eta_phi);
void sortHitsOnModules(std::vector<std::vector<HTTHit> >& hitsPerModule);
bool sortITkInputEta(const HTTHit& hitA, const HTTHit& hitB);
bool sortITkInputPhi(const HTTHit& hitA, const HTTHit& HitB);
bool etaOrPhi(const HTTHit& hit);

bool sortIBLInput(const std::unique_ptr<HTTHit>& i, const std::unique_ptr<HTTHit>& j);
bool sortPixelInput(const std::unique_ptr<HTTHit>& i, const  std::unique_ptr<HTTHit>& j);
...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
StatusCode HTTClusteringTool::initialize(){

  ATH_MSG_INFO ("HTTClusteringTool::initialize()");
  return StatusCode::SUCCESS;

}


StatusCode HTTClusteringTool::finalize(){
  ATH_MSG_INFO ("HTTClusteringTool::finalize()");
  return StatusCode::SUCCESS;
}
...
void HTTClusteringTool::Clustering(std::vector<HTTHit> moduleHits, std::vector<HTTCluster> &moduleClusters) const {
  ...
  for(auto& hit: moduleHits){
    ...
    for(auto& cluster:moduleClusters){
      if(hit.isPixel()){
	is_clustered_hit = updatePixelCluster(cluster, hit, false);
      }
      if(hit.isStrip()){
	is_clustered_hit = updateStripCluster(cluster, hit, false);
      }
    }
    ...
    if((is_clustered_hit==0) or (moduleClusters.size()==0)){
      HTTCluster cluster;
      if(hit.isPixel()){
	is_clustered_hit = updatePixelCluster(cluster, hit, true);
      }
      if(hit.isStrip()){
	is_clustered_hit = updateStripCluster(cluster, hit, true);
      }
...
void sortHitsOnModules(std::vector<std::vector<HTTHit> > &hitsPerModule, int &eta_phi){
  ...
  for (auto& module:hitsPerModule){
    ...
    if(etaOrPhi(module.at(0)) == true){
      //Sort by ETA first
      eta_phi = ETA;
      if (module.size() > 1) std::stable_sort(module.begin(), module.end(), sortITkInputEta);
      if (module.size() > 1) std::stable_sort(module.begin(), module.end(), sortITkInputPhi);
    } else {
      //Sort by PHI first
      eta_phi = PHI;
      if (module.size() > 1) std::stable_sort(module.begin(), module.end(), sortITkInputPhi);
...
void attachTruth(std::vector<HTTHit> &hits){
  for(auto& hit : hits) {
    HTTMultiTruth mt;
