...
{
  declareProperty("InFileName",   m_inpath, "wrapper file paths");
  declareProperty("RWstatus",     m_rwoption="READ", "define read or write file option:  READ,  RECREATE, HEADER");
}
...
StatusCode HTTInputHeaderTool::initialize(){
  ATH_MSG_INFO ("HTTInputHeaderTool::initialize()");
  ...
  if (m_rwoption.value()==std::string("READ") ) {
    ...
    m_EventTree->Branch(branchName.c_str(),
            branchName.c_str(),// class name
            &m_eventHeader);
  }
  else {
