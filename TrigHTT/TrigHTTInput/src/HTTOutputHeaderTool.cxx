...
{
  declareProperty("InFileName",           m_inpath,             "input file paths");
  declareProperty("RWstatus",             m_rwoption,           "define read or write file option: READ, RECREATE, HEADER");
...
StatusCode HTTOutputHeaderTool::openFile(std::string const & path)
{
  ...
  if (m_rwoption.value() == std::string("READ")) {
    ...
    // branches
    if (!m_EventTree->GetListOfBranches()->FindObject(branchNameIn_1st.c_str())) {
      ATH_MSG_FATAL ("Branch: " << branchNameIn_1st << " not found!");
      return StatusCode::FAILURE;
    }
    TBranch *branchIn_1st = m_EventTree->GetBranch(branchNameIn_1st.c_str());
    branchIn_1st->SetAddress(&m_eventInputHeader_1st);
    m_EventTree->SetBranchStatus(branchNameIn_1st.c_str(), 1);

    if (m_runSecondStage) {
      if (!m_EventTree->GetListOfBranches()->FindObject(branchNameIn_2nd.c_str())) {
        ATH_MSG_FATAL ("Branch: " << branchNameIn_2nd << " not found!");
        return StatusCode::FAILURE;
      }
      TBranch *branchIn_2nd = m_EventTree->GetBranch(branchNameIn_2nd.c_str());
      branchIn_2nd->SetAddress(&m_eventInputHeader_2nd);
      m_EventTree->SetBranchStatus(branchNameIn_2nd.c_str(), 1);
    }

    if (!m_EventTree->GetListOfBranches()->FindObject(branchNameOut.c_str())) {
      ATH_MSG_FATAL ("Branch: " << branchNameOut << " not found!");
      return StatusCode::FAILURE;
    }
    TBranch *branchOut = m_EventTree->GetBranch(branchNameOut.c_str());
    branchOut->SetAddress(&m_eventOutputHeader);
    m_EventTree->SetBranchStatus(branchNameOut.c_str(),1);
  }  
...
{
  ATH_MSG_INFO ("HTTOutputHeaderTool::initialize()");
  ...
  if (m_rwoption.value() == std::string("READ")) {
    ...
    m_EventTree->Branch(branchNameIn_1st.c_str(),
            "HTTLogicalEventInputHeader", // class name
            &m_eventInputHeader_1st);
    if (m_runSecondStage) {
      m_EventTree->Branch(branchNameIn_2nd.c_str(),
            "HTTLogicalEventInputHeader", // class name
            &m_eventInputHeader_2nd);
    }
    m_EventTree->Branch(branchNameOut.c_str(),
...
StatusCode HTTOutputHeaderTool::readData(HTTLogicalEventInputHeader* INheader_1st, HTTLogicalEventInputHeader* INheader_2nd, HTTLogicalEventOutputHeader* OUTheader, bool &last) 
{
  ...
  Int_t statIn_1st = m_EventTree->GetBranch(branchNameIn_1st.c_str())->GetEntry(m_event);
  Int_t statOut    = m_EventTree->GetBranch(branchNameOut.c_str())->GetEntry(m_event);

  if (statIn_1st <= 0) ATH_MSG_WARNING("Error in reading from branch " << branchNameIn_1st);
  if (statOut    <= 0) ATH_MSG_WARNING("Error in reading from branch " << branchNameOut);

  if (m_runSecondStage) {
    Int_t statIn_2nd = m_EventTree->GetBranch(branchNameIn_2nd.c_str())->GetEntry(m_event);
    if (statIn_2nd <= 0) ATH_MSG_WARNING("Error in reading from branch " << branchNameIn_2nd);
  }
