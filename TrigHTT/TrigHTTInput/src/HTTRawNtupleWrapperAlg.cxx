  ...
  m_ntowers(1)//64)
{
  ...
  declareProperty("OutFileName",  m_outpath= "httsim_Ntuplewrapper.root", "output path");
  declareProperty("GetOffline",   m_getOffline = false, "flag to enable the offline tracking save");
}


StatusCode HTTRawNtupleWrapperAlg::initialize()
{
  ATH_MSG_DEBUG ("HTTRawNtupleWrapperAlg::initialize()");

  ATH_CHECK(m_hitInputTool.retrieve());
  ATH_MSG_INFO ( "Setting HTT_SGHitInput tool" );
