...
static const InterfaceID IID_HTT_RawToLogicalHitsTool("HTT_RawToLogicalHitsTool", 1, 0);
const InterfaceID& HTT_RawToLogicalHitsTool::interfaceID( )
{ return IID_HTT_RawToLogicalHitsTool; }



HTT_RawToLogicalHitsTool::HTT_RawToLogicalHitsTool(const std::string& algname, const std::string &name, const IInterface *ifc) :
  AthAlgTool(algname,name,ifc),
  m_HTTMapping("TrigHTTMappingSvc", name),
  m_EvtSel("HTTEventSelectionSvc", name)
{
  declareInterface<HTT_RawToLogicalHitsTool>(this);// needed for rel20
  declareProperty("TowersToMap",      m_towersToMap,          "Which Towers to map, goes from 0 to 96!");
  declareProperty("SaveOptional",     m_saveOptional = 2,     "flag to enable the truth/offline tracking save");
  // declareProperty("HTTMappingSvc",    m_HTTMapping);
}


StatusCode HTT_RawToLogicalHitsTool::initialize(){
...
StatusCode HTT_RawToLogicalHitsTool::convert(unsigned stage, const HTTEventInputHeader& eventHeader, HTTLogicalEventInputHeader& logicEventHeader){
  ...
  for (auto hit: eventHeader.hits()) { // hit loop
      ...
      if (!mapped) {
          ...
          //std::cout<<"Current hit cannot be mapped in a Tower: type="<<hit.getHitType()<<" BarreLEC="<<hit.getBarrelEC() <<" Layer="<<hit.getLayer() <<std::endl;
...
StatusCode HTT_RawToLogicalHitsTool::getUnmapped(std::vector<HTTHit>& missing_hits) {
    copy(m_missing_hits.begin(), m_missing_hits.end(), back_inserter(missing_hits));
    //  missing_hits = m_missing_hits;
    return StatusCode::SUCCESS;
}

const HTTPlaneMap* HTT_RawToLogicalHitsTool::getPlaneMap_1st() {
  return m_HTTMapping->PlaneMap_1st();
}

StatusCode HTT_RawToLogicalHitsTool::finalize(){
    ATH_MSG_INFO ( "HTT_RawToLogicalHitsTool::finalized()");
    return StatusCode::SUCCESS;
}
