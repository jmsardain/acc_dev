 ...
 *      - logiLayer: A logical layer corresponding to an AM bus, i.e. the number of SSIDs in a pattern. Called 'plane' in FTK.
 *              In general, `layer` with no other specifiers refers to a logical layer.
 */
...
struct LayerSection
{
    int layer; // logical layer, -1 if unused
    uint32_t section; // arbitrary enumeration (identifier) if there are multiple detector elements in the same logical layer
};
 ...
 * ^ ^ ^
 * Is this still true in HTT?
 */
struct LayerInfo
{
    SiliconTech siTech;
    DetectorZone zone;
    int physLayer;	// physical layer number (see this file's header)
    int physDisk;	// ???
    int stereo;     // sct layer side
};


class HTTPlaneMap : public AthMessaging
{
    public:
        ...
        HTTPlaneMap(MsgStream & msg, const std::string & filepath, unsigned region, unsigned stage,
