    ...
    : base_class(algname, name, ifc)
{
    ...
    declareProperty("PhiWindow", m_phiwindow, "Distance in phi to consider two hits for making a space-point");
    declareProperty("Duplication", m_duplicate, "Duplicate spacepoint to layer on the other side of the stave");
    declareProperty("Filtering", m_filter, "Filter out incomplete spacepoints");
    declareProperty("FilteringClosePoints", m_filterClose, "Filter out single hits close to spacepoints");
}

// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
StatusCode HTTSpacePointsTool_v2::initialize()
{
    ATH_MSG_INFO("Using spacepoints v2");
...
void HTTSpacePointsTool_v2::calcPosition(HTTHit &hit_in, HTTHit &hit_out, float &x, float &y, float &z)
{
	...
	if (hit_in.getHTTEtaModule()==hit_in.getHTTEtaModule()){
	  ...
	  float m_window = 3; //mm, max possible distance between hits from same track with reasonable incline to module
	  float r_limited = std::max(r_sp,r_bounds[hit_in.getHTTEtaModule()+1]-m_window);
	  r_limited = std::min(r_sp,r_bounds[hit_in.getHTTEtaModule()+1]+m_window);
	  if (r_sp!=r_limited) {
	    ATH_MSG_WARNING("Crossing spacepoint location too far from module boundary: r_sp=" << r_sp
			    << " not in [" << r_bounds[hit_in.getHTTEtaModule()+1]-m_window << "," << r_bounds[hit_in.getHTTEtaModule()+1]+m_window  << "]");
	    r_sp=r_limited;
	  }
	}
