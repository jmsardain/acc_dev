...
HTTPlaneMap::HTTPlaneMap(MsgStream & msg, const std::string & filepath, unsigned region, unsigned stage, std::vector<int> layerOverrides) :
    AthMessaging(msg),
    ...
    m_layerOverrides(layerOverrides)
{
    ...
    ATH_MSG_INFO("Using " << m_nLogiLayers << " logical layers and " << m_nCoords << " coordinates");
}
...
void HTTPlaneMap::allocateMap(ifstream & fin, uint32_t stage)
{
    ...
    else ATH_MSG_ERROR("Bad geometry version " << geoKey);

    ATH_MSG_INFO("Allocating map for geometry " << geoKey <<" diskIndex size="<<m_diskIndex.size());
    ...
    else ATH_MSG_ERROR("Bad stage " << stage << ". Stage should be 1 or 2");
    ...
    if (!ok) ATH_MSG_FATAL("Error reading layer counts");
...
void HTTPlaneMap::seek(ifstream & fin, unsigned region)
{
    ...
    ATH_MSG_DEBUG("seek() ended at:" << line);
}
...
void HTTPlaneMap::readLayers(ifstream & fin, uint32_t stage)
{
    ...
    while (getline(fin, line))
    {
        ...
        else ATH_MSG_FATAL("Bad Detector zone" << BEC);
        ...
        else ATH_MSG_FATAL("Bad silicon type " << silicon);
        ...
        linesRead++;
        if (logiLayer >= (int)m_nLogiLayers) ATH_MSG_FATAL("Logical layer " << logiLayer << " exceeds expected number " << m_nLogiLayers);
    ...
    if (!ok) ATH_MSG_FATAL("Error reading file");
}
...
void HTTPlaneMap::map(HTTHit & hit) const
{
    ...
    if (hit.isPixel() && !hit.isBarrel()) {
        ...
        else ATH_MSG_ERROR("Error: requesting "<< hit.getPhysLayer() <<" element in m_diskIndex which is of size "<< m_diskIndex.size() );
    }
