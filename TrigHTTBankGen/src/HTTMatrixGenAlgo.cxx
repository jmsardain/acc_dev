...
HTTMatrixGenAlgo::HTTMatrixGenAlgo(const std::string& name, ISvcLocator* pSvcLocator) :
    ...
    m_hitInputTool("HTTSGToRawHitsTool/HTTSGToRawHits"),
    m_hitMapTool("HTT_RawToLogicalHitsTool/HTT_RawToLogicalHitsTool"),
    m_clusteringTool("HTTClusteringTool/HTTClusteringTool")
{
    declareProperty("NBanks",m_nRegions);
    declareProperty("Clustering",m_doClustering);
    declareProperty("IdealiseGeometry",m_ideal_geom);
    declareProperty("SingleSector",m_single);
    declareProperty("HoughConstants",m_doHoughConstants);
    declareProperty("RoadFinder",m_roadFinderTool);

    // slices
    declareProperty("par_c_max",m_sliceMax.qOverPt);
    declareProperty("par_c_min",m_sliceMin.qOverPt);
    declareProperty("par_c_slices",m_nBins.qOverPt);
    declareProperty("par_phi_max",m_sliceMax.phi);
    declareProperty("par_phi_min",m_sliceMin.phi);
    declareProperty("par_phi_slices",m_nBins.phi);
    declareProperty("par_d0_max",m_sliceMax.d0);
    declareProperty("par_d0_min",m_sliceMin.d0);
    declareProperty("par_d0_slices",m_nBins.d0);
    declareProperty("par_z0_max",m_sliceMax.z0);
    declareProperty("par_z0_min",m_sliceMin.z0);
    declareProperty("par_z0_slices",m_nBins.z0);
    declareProperty("par_eta_max",m_sliceMax.eta);
    declareProperty("par_eta_min",m_sliceMin.eta);
    declareProperty("par_eta_slices",m_nBins.eta);

    // track filtering
    declareProperty("PT_THRESHOLD",m_PT_THRESHOLD);
    declareProperty("D0_THRESHOLD",m_D0_THRESHOLD);
    declareProperty("TRAIN_PDG",m_TRAIN_PDG);

    declareProperty("WCmax",m_MaxWC);
}
...
StatusCode HTTMatrixGenAlgo::bookHistograms()
{
    ...
    for (unsigned i = 0; i < HTTTrackPars::NPARS; i++)
    {
        ...
        h_trainingTrack[i] = new TH1I(name.c_str(), title.c_str(), 100, m_sliceMin[i], m_sliceMax[i]);
        ATH_CHECK(m_tHistSvc->regHist("/TRIGHTTMATRIXOUT/" + name, h_trainingTrack[i]));
    }
    ...
    for (unsigned i = 0; i < HTTTrackPars::NPARS; i++)
    {
        ...
        h_sectorPars[i] = new TH1I(name.c_str(), title.c_str(), 100, m_sliceMin[i], m_sliceMax[i]);
        ATH_CHECK(m_tHistSvc->regHist("/TRIGHTTMATRIXOUT/" + name, h_sectorPars[i]));
    }
    ...
    for (unsigned i = 0; i < HTTTrackPars::NPARS; i++)
    {
        ...
        h_SHfailure[i] = new TH1I(name.c_str(), title.c_str(), 100, m_sliceMin[i], m_sliceMax[i]);
        ATH_CHECK(m_tHistSvc->regHist("/TRIGHTTMATRIXOUT/" + name, h_SHfailure[i]));
    }
    ...
    for (unsigned i = 0; i < HTTTrackPars::NPARS; i++)
    {
        ...
        h_3hitsInLayer[i] = new TH1I(name.c_str(), title.c_str(), 100, m_sliceMin[i], m_sliceMax[i]);
        ATH_CHECK(m_tHistSvc->regHist("/TRIGHTTMATRIXOUT/" + name, h_3hitsInLayer[i]));
    }
    ...
    for (unsigned i = 0; i < HTTTrackPars::NPARS; i++)
    {
        ...
        h_notEnoughHits[i] = new TH1I(name.c_str(), title.c_str(), 100, m_sliceMin[i], m_sliceMax[i]);
        ATH_CHECK(m_tHistSvc->regHist("/TRIGHTTMATRIXOUT/" + name, h_notEnoughHits[i]));
    }

    h_trackQoP_okHits = new TH1I("h_trackQoP_okHits", "half inverse pt of tracks passing hit check",
            m_nBins.qOverPt, m_sliceMin.qOverPt, m_sliceMax.qOverPt);
    h_trackQoP_okRegion = new TH1I("h_trackQoP_okRegion", "half inverse pt of tracks passing region check",
            m_nBins.qOverPt, m_sliceMin.qOverPt, m_sliceMax.qOverPt);
    h_nHit = new TH1I("h_nHit", "number of hits in sector", 100, 0, 100);

    ATH_CHECK(m_tHistSvc->regHist("/TRIGHTTMATRIXOUT/h_trackQoP_okHits", h_trackQoP_okHits));
    ATH_CHECK(m_tHistSvc->regHist("/TRIGHTTMATRIXOUT/h_trackQoP_okRegion", h_trackQoP_okRegion));
...
StatusCode HTTMatrixGenAlgo::execute()
{
    ...
    HTTEventInputHeader header;
    bool last=false;
    ATH_CHECK(m_hitInputTool->readData(&header, last));
    if (last) return StatusCode::SUCCESS;

    std::vector<HTTHit> hits = getLogicalHits(header);

    std::vector<HTTTruthTrack> truth_tracks = header.optional().getTruthTracks();
    ...
    for (HTTTruthTrack const & track : tracks)
    {
        ...
        if (!success) continue; // Skip this track if it has bad hits (not 8/8, etc.)
        h_trackQoP_okHits->Fill(track.getQOverPt());

        // Get the region of this sector
        int region = getRegion(sector_hits);
        if (region < 0 || region >= m_nRegions) continue;
        h_trackQoP_okRegion->Fill(track.getQOverPt());
...
// TODO replace this with the RawToLogical tool
std::vector<HTTHit> HTTMatrixGenAlgo::getLogicalHits(HTTEventInputHeader & header) const
{
    ...
    unsigned stage = 0;
    if (m_doHoughConstants) stage = 1; // TODO only works on 1st stage
    else stage = 2;
    StatusCode sc = m_hitMapTool->convert(stage, header, logicalHeader);
...
std::vector<HTTTruthTrack> HTTMatrixGenAlgo::filterTrainingTracks(std::vector<HTTTruthTrack> const & truth_tracks) const
{
	...
	if (abs(track.getD0()) > m_D0_THRESHOLD) continue;
        ...
        fillTrackPars(h_trainingTrack, track);
    }
  ...
  if ((new_hit.getSection() == old_hit.getSection()) && (new_hit.getLayer() == old_hit.getLayer())
    ...
    if (old_section == new_section)
    {
		...
		int rmax = phi_max - phi_min;
            ...
            if (phi_diff == 1 || phi_diff == -rmax) return SH_KEEP_OLD;
            else if (phi_diff == -1 || phi_diff == rmax) return SH_KEEP_NEW;
            else
            {
	...
	if (m_doHoughConstants) {
        ...
        {
            if (old_isEC) return SH_KEEP_NEW;
            else return SH_KEEP_OLD;
        }
        ...
        else
        {
            ...
            if (old_disk > new_disk) return SH_KEEP_NEW;
            else return SH_KEEP_OLD;
        }
    }
}
        ...
        /* TEMP */ HTTTruthTrack const & t) const
{
    ...
    for (HTTHit const & hit : all_hits)
    {
        ...
        else if (layer_count[layer] == 1)
        {
            ...
            if (selected_hit == SH_FAILURE)
            {
                fillTrackPars(h_SHfailure, t);
                return false;
            }
            else if (selected_hit == SH_KEEP_NEW) sector_hits[layer] = hit;
        }
        else
        {
            ATH_MSG_DEBUG("Too many hits on a plane, exiting filterHitsSec");
            fillTrackPars(h_3hitsInLayer, t);
            return false;
        }
    }
    ...
    if (nwc > m_MaxWC)
    {
        fillTrackPars(h_notEnoughHits, t);
        return false;
    }
    return true;
}
...
StatusCode HTTMatrixGenAlgo::makeAccumulator(std::vector<HTTHit> const & sector_hits, HTTTruthTrack const & track, std::pair<std::vector<module_t>, HTTMatrixAccumulator> & accumulator) const
{
	...
	else modules[i] = sectorbin; // JAAAAA here we used to set the identifier, now just global zero! we can change this by large region if needed?
      }
      else {
        modules[i] = -1; // WC
      }
    ...
    double const houghRho = HTTHoughTransformTool::A * y; // Aq/pT
    ...
    {
      if (sector_hits[i].getHitType() != HitType::wildcard) {
        double hitGPhi = sector_hits[i].getGPhi(); // need to be careful about 2 pi boundary?
...
StatusCode HTTMatrixGenAlgo::finalize()
{
    ...
    for (int region = 0; region < m_nRegions; region++)
    {
        ...
        for (auto & sector_info : m_sector_cum[region])
        {
            ...
            for (unsigned i = 0; i < HTTTrackPars::NPARS; i++)
                h_sectorPars[i]->Fill(sector_info.second.pars[i] / coverage);
        }
    }
