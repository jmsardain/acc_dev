...
void HTTMultiTruth::display() const
{
    ...
    std::cout << padding << "HTTMultiTruth (event index,barcode) <=> weight, entries: " << m_truth.size() << std::endl;

    if (m_truth.empty()) return;

    for (auto const& truth : m_truth)
        std::cout << padding << "   " << (truth.first).first << "," << (truth.first).second << " <=> " << truth.second << std::endl;

    std::cout << padding << "      best: ";

    Barcode code;
    Weight weight(0);

    if (!best(code, weight)) std::cout << "N/A";
    else std::cout << code.first << "," << code.second << " " << weight;
