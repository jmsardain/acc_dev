...
double rms95(TH1 const * h)
{
    ...
    int i = 1;
    while (true) {
        int _lowerbin = meanbin - i;
        int _upperbin = meanbin + i;
        if (_lowerbin < 1 || _upperbin > h->GetNbinsX()) break;

        sum += h_tmp->GetBinContent(_lowerbin) + h_tmp->GetBinContent(_upperbin);
